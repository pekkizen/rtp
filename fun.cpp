...
// This is Hoare's quicksort partition with external pivot.
...
// Equivalent C++ function: std::nth_element(p.begin(), p.begin() + k, p.end());
...
static void uniSelect(int k, NumericVector p) {
    ...
    if (pi + 1 < k) { // add k-pi-1 numbers to pi+1, ...
