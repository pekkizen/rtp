...
// select swaps k smallest values in array p to p[0], ..., p[k-1].
// For small k and large n this is O(n).
static void select(int k, int n, NumericVector p) {
    if (k <= 0 || k >= n) return;
...
// This is known as Hoare's quicksort partition.
...
// This is efficient if numbers are near Uniform(0, 1) distributed.
static void selectSmalls(int k, NumericVector p) {
    int n = p.size();
    if (k <= 0 || k >= n) return;
    ...
    double SD = betaSD(k, n - k + 1);
    double mean = betaMean(k, n - k + 1);
    double pivot = mean + SD;

    int ip = 0, lo = 0;
    while (ip < k - 1) {
        ip = partition(lo, n - 1, pivot, p);
        pivot += SD;
